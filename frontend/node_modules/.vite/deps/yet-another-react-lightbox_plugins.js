...
function ThumbnailsTrack({ visible, containerRef }) {
  ...
  return React8.createElement(
    ...
    React8.createElement("nav", { ref: track, style: styles.thumbnailsTrack, className: clsx(cssClass(cssPrefix2("track")), cssClass(CLASS_FLEX_CENTER)), tabIndex: -1, ...registerSensors }, items.map(({ key, index, slide }) => {
      ...
      return React8.createElement(Thumbnail, { key, slide, active: index === globalIndex, fadeIn, fadeOut, placeholder: !slide, onClick: handleClick(index), onLoseFocus: () => {
        var _a;
...
function useZoomState(imageRect, maxZoom, zoomWrapperRef) {
  ...
  const zoomIn = React10.useCallback(() => changeZoom(zoom * zoomInMultiplier), [zoom, zoomInMultiplier, changeZoom]);
  const zoomOut = React10.useCallback(() => changeZoom(zoom / zoomInMultiplier), [zoom, zoomInMultiplier, changeZoom]);
...
function ZoomContextProvider({ children }) {
  ...
  const zoomRef = React10.useMemo(() => ({ zoom, maxZoom, offsetX, offsetY, disabled, zoomIn, zoomOut, changeZoom }), [zoom, maxZoom, offsetX, offsetY, disabled, zoomIn, zoomOut, changeZoom]);
  React10.useImperativeHandle(useZoomProps().ref, () => zoomRef, [zoomRef]);
